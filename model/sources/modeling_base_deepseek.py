...
from model.sources.model_config import Qwen2_5_Config
logger = logging.getLogger(__name__)

from transformers import LlamaTokenizer,AutoTokenizer,AutoModel,AutoModelForCausalLM,AutoProcessor
from transformers import AutoConfig, PreTrainedModel, Qwen2ForCausalLM
...
class BaseMLLM(PreTrainedModel):
    ...
    def build_bridge(self):
        # ViT to LM: 768 -> 4096 -> 5120
        self.project_up = nn.Linear(768, 4096)
        self.project_up_deepseek = nn.Linear(4096, 5120, bias=False)
        nn.init.xavier_uniform_(self.project_up_deepseek.weight)
        
        # LM to ViT: 5120 -> 4096 -> 768 
        self.project_down_deepseek = nn.Linear(5120, 4096, bias=False)
        nn.init.xavier_uniform_(self.project_down_deepseek.weight)
        self.project_down = nn.Linear(4096, 768)
    ...
    def build_llm(self):
        ...
        if self.model_config.llm.name == 'mistral_7b':
            ...
            # 1. 모델 로딩
            # - pretrained_llm_path에서 모델을 로드
            # - trust_remote_code=True: 원격 코드 신뢰 
            # - use_safetensors=True: 안전한 텐서 저장 형식 사용
            ...
            # 2. 모델 상태 사전 검증
            # - original_state_dict: 원본 모델의 가중치/매개변수 저장
            original_state_dict = self.lm.state_dict()
            
            # 3. 키 이름 변환
            # - 'lm.model'을 'model'로 변경하여 새로운 state_dict 생성 
            expected_keys = set(original_state_dict.keys())
            new_state_dict = {k.replace('lm.model', 'model'): v 
                            for k, v in original_state_dict.items()}
            
            # 4. 모델 로딩 검증
            # - strict=False: 완벽한 키 매칭이 아니어도 로딩 허용
            # - missing_keys: 누락된 키 목록
            # - unexpected_keys: 예상치 못한 키 목록 
            missing_keys, unexpected_keys = self.lm.load_state_dict(new_state_dict, strict=False)
            
            # print(f"self.lm.state_dict().keys(): {self.lm.state_dict().keys()}")
            # 5. 검증 결과 로깅
            # - 누락되거나 예상치 못한 키가 있다면 경고 메시지 출력
            if missing_keys:
                logger.warning(f"Missing keys after remapping: {missing_keys}")
            if unexpected_keys:
                logger.warning(f"Unexpected keys after remapping: {unexpected_keys}")
        ...
        # 01.18, deamin
        self.freeze_llm = self.model_config.freeze_llm # True->False->True: OOM 문제
        ...
        # 비전 임베딩과의 차원 일치 검증
        if 5120 != self.lm.config.hidden_size:
            raise ValueError(f"Projection layer output dim {5120} "
                                f"must match LLM hidden size {self.lm.config.hidden_size}")

        # 상태 사전 매핑 검증 개선
        expected_keys = ['model.embed_tokens.weight', 'model.layers.0.self_attn.q_proj.weight']  # 실제 모델 구조에 맞는 키 예시
        for key in expected_keys:
            if key not in self.lm.state_dict():
                raise KeyError(f"Critical model weight {key} not found in state dict")
