...
import numpy as np
from sampling import read_frames_cv2, read_frame_cv2, read_frame_offset_cv2
# 01.18, deamin
...
class InternVideo2_VideoChat2__Image_Offset_Dataset(Dataset): 
    '''
    InternVideo2_VideoChat2_Image_Offset_Dataset 클래스
    segment를 불러오고, frame 변환 및 preprocess를 진행하여 Tensor로 반환한다.
    ...
    Args:
        ...
        audio_reader_type: str, 오디오 로더 타입
        train: bool, 학습 여부
        offset: int, 등간격으로 몇 개의 프레임을 추출하여 모델에 넣어줄 지 결정
        save_frames_as_img: bool, 샘플링되는 프레임 저장 여부
    ...
    def __getitem__(self, index):
        ...
        # 동영상 읽기
        frames, frame_indices, duration = read_frame_offset_cv2(
            video_path, self.use_segment, start_time, end_time, self.s3_client, offset = self.offset, save_frames_as_img = self.save_frames_as_img)
...
# 01.18, deamin
from torch.utils.data import DataLoader
class InternVideo2_VideoChat2_Image_Offset_DataLoader(DataLoader):
    def __init__(
        self,
        dataset: InternVideo2_VideoChat2_Dataset,
        batch_size: int = 2,
