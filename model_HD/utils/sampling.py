...
def read_frame_cv2(
    ...
    if save_frames_as_img:
        temp_path = os.path.join('./saved_frames', os.path.splitext(os.path.basename(video_path))[0])
        os.makedirs(temp_path, exist_ok=True)
        output_path = os.path.join(temp_path, f"{os.path.basename(video_path)}_{frame_index:04d}.png")  # 예: frame_0010.png
        cv2.imwrite(output_path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))  # 저장 시 RGB -> BGR 변환 필요
        print(f"success processing frame idx: {frame_index} frames")
    video.release()
    

    # (T, H, W, C) to (T, C, H, W), numpy to tensor, dtype=torch.uint8
    frames = torch.tensor(frames, dtype=torch.uint8).permute(0, 3, 1, 2)
