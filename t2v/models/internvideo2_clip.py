...
class InternVideo2_CLIP(nn.Module):
    ...
    @torch.no_grad()
    def get_video_embedding(self, video_tensor):
        """
        비디오 임베딩 추출 메서드
        Input: [Batch, Time, Channel, Height, Width] 형태의 텐서
        Output: L2 정규화된 [Batch, Embedding_dim] 형태의 임베딩
        """
        self.eval()
        vision_embeds = self.encode_vision(video_tensor, test=True)
        return nn.functional.normalize(vision_embeds, p=2, dim=-1)

    @torch.no_grad()
    def get_text_embedding(self, text_input):
        """
        텍스트 임베딩 추출 메서드
        Input: tokenizer로 처리된 텍스트 입력
        Output: L2 정규화된 [Batch, Embedding_dim] 형태의 임베딩
        """
        self.eval()
        text_embeds = self.encode_text(text_input)
        return nn.functional.normalize(text_embeds, p=2, dim=-1)
