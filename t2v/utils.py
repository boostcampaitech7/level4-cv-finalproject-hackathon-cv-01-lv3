...
from models.backbones.bert.tokenization_bert import CustomBertTokenizer
...
def retrieve_video(videos, text, model, topk:int=3, device=torch.device('cuda')):
    """
    ...
    text_feat = vlm.get_txt_feat(text)
    video_sims = []  # 유사도 점수 저장 리스트
    for video in videos:
        video = cv2.VideoCapture(video)
        frames = [frame for frame in _frame_from_video(video)]
        frames_tensor = frames2tensor(frames, fnum=4, target_size=(224, 224), device=device)
        print(f"frames_tensor.shape: {frames_tensor.shape}")
        vid_feat = vlm.get_vid_feat(frames_tensor)
        print(f"vid_feat.shape: {vid_feat.shape}")
        # print("비전 임베드", vid_feat[0].shape)
        # print("풀드 임베드", vid_feat[1].shape)
        # 유사도 계산
        sim_score = (vid_feat @ text_feat.T).squeeze()  # softmax 적용 전 원점수
        video_sims.append(sim_score)
    
    # 전체 점수에 대해 softmax 적용
    video_probs = F.softmax(100.0 * torch.stack(video_sims), dim=-1)
    top_probs, top_indices = video_probs.topk(topk, dim=-1)
    
    # 비디오 경로 매핑
    top_videos = [videos[i] for i in top_indices.cpu().numpy()]
    # 결과 출력
    print(f"Top {topk} 비디오:")
    for video, prob in zip(top_videos, top_probs):
        print(f"비디오: {video}, 유사도: {prob:.4f}")
    return top_videos, top_probs
...
def setup_internvideo2(config: dict):
    if "bert" in config.model.text_encoder.name:
        tokenizer = CustomBertTokenizer.from_pretrained(config.model.text_encoder.pretrained, local_files_only=False)
...
def extract_vid_embedding(frames, model, config, device):
    # 프레임 텐서 변환
    fn = config.get('num_frames', 8)
    size_t = config.get('size_t', 224)
    frames_tensor = frames2tensor(frames, fnum=fn, target_size=(size_t, size_t), device=device)
    ...
    # 1. 비전 특징 추출 (프로젝션 제외)
    with torch.no_grad():
        vision_embeds, pooled_vision_embeds = model.encode_vision(frames_tensor, test=True)
    
    # 2. 차원 검증
    assert model.vision_proj.in_features == vision_embeds.size(-1), \
        f"차원 불일치: {vision_embeds.size(-1)} vs {model.vision_proj.in_features}"
    
    # 3. 프로젝션 및 정규화
    pooled_vision_embeds = F.adaptive_avg_pool1d(vision_embeds.mean(1), 1)  # 시간 축 평균 풀링
    vision_feats = model.vision_proj(pooled_vision_embeds)
    vision_feats = F.normalize(vision_feats, p=2, dim=-1)   
    
    return vision_feats.detach().cpu().numpy()

def extract_txt_embedding(text, 
                         model,
                         tokenizer,
                         device=torch.device('cuda')):
    with torch.no_grad():
        text_feats = model.get_txt_feat(text)  # raw text 직접 전달
