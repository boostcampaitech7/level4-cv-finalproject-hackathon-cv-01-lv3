import torch
from weights.LongCLIP.model import longclip
import numpy as np
import cv2
...
class VideoCaption:
    def __init__(self):
        # 설정 로드
        self.model, self.preprocess = longclip.load("./weights/LongCLIP/checkpoints/longclip-L.pt", DEVICE)
...
def search_videos_clip(query_text: str):
    start = time.time()
    ...
    try:
        ...
        for i, hit in enumerate(results['hits']['hits']):
            ...
            if inner_hits:
                ...
                print(f"{i}번째: {score}, {source['segment_name']}")
                results.append((source['segment_name'], score)) # TODO 나중에 최종 반환
                if score > max_score:
                    max_score = score
...
def run(query_text: str)-> tuple[str, str]:
    results, segment_name = search_videos_clip(query_text)
...
def calculate_r1_r5_r10(csv_path):
    ...
    for _, row in tqdm(df.iterrows(), total=total_queries, desc="Calculating R@1, R@5, R@10", unit="query"):
        ...
        print(f"GT: {true_segment_name}")
        
        predicted_segments, embedding_time, searching_time, total_time = search_videos_clip(query)  
